...
let apiVesselsSeen = 0;
let apiVesselsParsed = 0;
let lastParsedMmsi = null;
...
function consume(source,d){
  lastFrameType=d?.type||null;
  if(d?.type==='notice'){ lastNotice={code:d.code||null,message:d.message||null,at:new Date().toISOString()}; return null; }
  const v=normalizePelyr(d);
  if(!v){ ignoredMessages++; return null; }
  parsedMessages++; lastParsedMmsi=v.mmsi; lastParsedAt=new Date().toISOString();
  return mergeVessel(v);
}
...
async function refreshPelyrApi(){
  ...
  for(const b of BOXES){
    ...
    try{
      ...
      s.lastApiError=null;
      const items=Array.isArray(r.body?.vessels)?r.body.vessels:[];
      apiVesselsSeen+=items.length;
      for(const item of items){const v=normalizePelyrApi(item);if(v){apiVesselsParsed++;lastParsedMmsi=v.mmsi;lastParsedAt=new Date().toISOString();mergeVessel(v);}}
...
function streamHealth(s){return {connected:s.connected,subscriptionConfirmed:s.subscriptionConfirmed,totalMessages:s.totalMessages,lastMessageAt:s.lastMessageAt,connectedAt:s.connectedAt,disconnectedAt:s.disconnectedAt,reconnectAttempt:s.reconnectAttempt,nextReconnectAt:s.nextReconnectAt,lastClose:s.lastClose,lastError:s.lastError,successfulSubscriptions:s.successfulSubscriptions,state:s.state||undefined,cooldownUntil:s.cooldownUntil||undefined,lastHandshake:s.lastHandshake,lastHeartbeat:s.lastHeartbeat||null,dropped:s.dropped||0,lastApiRefreshAt:s.lastApiRefreshAt||null,lastApiStatus:s.lastApiStatus||null,lastApiError:s.lastApiError||null};}
app.get('/',(req,res)=>res.json({ok:true,version:'10.8.0',name:'MaritimeScope Pelyr AIS Bridge',providers:['Pelyr OPEN-AIS'],mode:PUSH_INGEST?'pelyr-push-ingest':'pelyr-pull',testMode:!ALLOW_GLOBAL_BOXES,boxes:BOXES,endpoints:['/health','/diagnostics','/vessels','/vessel','/search']}));
app.get('/health',(req,res)=>res.json({ok:true,version:'10.8.0',providers:{pelyr:streamHealth(streams.pelyr)},ingest:{enabled:PUSH_INGEST,pending:pending.size,successes:ingestSuccesses,failures:ingestFailures,rowsStored:ingestRowsStored,lastIngestAt,lastIngestStatus,lastIngestError,lastIngestDetail},totalMessages,cacheSize:cache.size,lastError,boxes:BOXES,testMode:!ALLOW_GLOBAL_BOXES}));
app.get('/diagnostics',(req,res)=>res.json({ok:true,version:'10.8.0',time:new Date().toISOString(),config:{pelyrKeyConfigured:!!PELYR_API_KEY,bridgeTokenConfigured:!!BRIDGE_TOKEN,pushIngestEnabled:PUSH_INGEST,allowGlobalBoxes:ALLOW_GLOBAL_BOXES,boxes:BOXES,maxCache:MAX_CACHE},providers:{pelyr:{...streamHealth(streams.pelyr),mode:'token',httpsApi:!!PELYR_API_KEY}},data:{totalMessages,parsedMessages,ignoredMessages,cacheSize:cache.size,pendingIngest:pending.size,apiVesselsSeen,apiVesselsParsed,lastParsedMmsi,lastParsedAt,lastFrameType,lastNotice}}));
...
app.get('/stats',(req,res)=>{if(!authorized(req,res))return;res.json({ok:true,cacheSize:cache.size,totalMessages,parsedMessages,ignoredMessages,apiVesselsSeen,apiVesselsParsed,lastParsedMmsi,lastParsedAt,lastFrameType,lastNotice,pelyr:streamHealth(streams.pelyr)});});
app.listen(PORT,'0.0.0.0',()=>{console.log(`MaritimeScope Pelyr AIS bridge v10.8 listening on ${PORT}`);console.log(`Pelyr: ${!!PELYR_API_KEY?'configured':'NOT configured'}`);console.log(`Boxes: ${JSON.stringify(BOXES)}`);setTimeout(()=>{connectPelyr();},PELYR_START_DELAY_MS); setInterval(()=>{refreshPelyrApi();},PELYR_API_REFRESH_MS); setTimeout(()=>{refreshPelyrApi();},PELYR_START_DELAY_MS+3000);});
