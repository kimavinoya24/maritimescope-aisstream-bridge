...
const AISSTREAM_START_DELAY_MS = Math.max(10000, Number(process.env.AISSTREAM_START_DELAY_MS || 60000));
const AISSTREAM_429_COOLDOWN_MS = Math.max(60000, Number(process.env.AISSTREAM_429_COOLDOWN_MS || 300000));
...
function handleUnexpectedResponse(source,response){const s=streams[source],chunks=[];s.connected=false;s.socket=null;response.on('data',c=>{if(Buffer.concat(chunks).length<4096)chunks.push(Buffer.from(c));});response.on('end',()=>{const body=Buffer.concat(chunks).toString('utf8').slice(0,4096);s.lastHandshake={status:response.statusCode,headers:safeHeaderSubset(response.headers),bodySnippet:body,at:new Date().toISOString()};const ra=response.statusCode===429?Math.max(AISSTREAM_429_COOLDOWN_MS,retryAfterMs(response)):retryAfterMs(response);s.lastError=`HTTP ${response.statusCode}${response.headers['retry-after']?`; Retry-After=${response.headers['retry-after']}`:''}`;
if(source==='aisstream'){s.state=response.statusCode===429?'rate_limited':'handshake_rejected';s.cooldownUntil=ra?new Date(Date.now()+ra).toISOString():null;lastError=`aisstream: ${s.lastError}`;}if(ra>0&&!s.reconnectTimer){s.reconnectAttempt++;s.nextReconnectAt=new Date(Date.now()+ra).toISOString();s.reconnectTimer=setTimeout(()=>{s.reconnectTimer=null;s.nextReconnectAt=null;connectSource(source);},ra);}else scheduleReconnect(source,s.lastError);});}
function handleClose(source,code,reasonBuffer){const s=streams[source],reason=Buffer.isBuffer(reasonBuffer)?reasonBuffer.toString('utf8'):String(reasonBuffer||'');s.lastClose={code,reason,at:new Date().toISOString()};s.disconnectedAt=s.lastClose.at;s.connected=false;s.socket=null;if(source==='aisstream'&&s.state!=='rate_limited')s.state='closed';const combined=(reason+' '+(s.lastError||'')).toLowerCase();if(source==='openwaters'&&OPENWATERS_API_KEY&&!s.anonymousFallback&&/invalid token|bad token|unauthorized|401|api key|token/.test(combined)){s.anonymousFallback=true;s.lastError='Open Waters token rejected; retrying anonymously';}scheduleReconnect(source,s.lastError||`WebSocket closed (${code})`);}
function openSocket(source,url,onOpen,onMessage){const s=streams[source];if(source==='aisstream'&&s.cooldownUntil&&Date.parse(s.cooldownUntil)>Date.now())return;s.totalConnectAttempts++;s.lastConnectStartedAt=new Date().toISOString();if(source==='aisstream'){s.state='connecting';s.cooldownUntil=null;}s.subscriptionConfirmed=false;s.compressionEnabled=null;s.lastHandshake={status:null,headers:{},bodySnippet:null,at:s.lastConnectStartedAt};try{const ws=new WebSocket(url,{perMessageDeflate:true,handshakeTimeout:15000,maxPayload:10*1024*1024});s.socket=ws;ws.on('open',()=>{s.connected=true;if(source==='aisstream')s.state='connected';s.connectedAt=new Date().toISOString();s.lastError='';onOpen(ws,s);});ws.on('unexpected-response',(_,res)=>handleUnexpectedResponse(source,res));ws.on('message',data=>{s.lastMessageAt=new Date().toISOString();s.totalMessages++;totalMessages++;try{onMessage(JSON.parse(Buffer.isBuffer(data)?data.toString('utf8'):String(data)),s);}catch(e){s.lastError='JSON: '+e.message;}});ws.on('error',e=>{s.lastError=e?.message||String(e);if(source==='aisstream')s.state='error';lastError=`${source}: ${s.lastError}`;});ws.on('close',(c,r)=>handleClose(source,c,r));}catch(e){s.socket=null;s.connected=false;s.lastError=e.message;scheduleReconnect(source,e.message);}}
...
function connectAISStream(){const s=streams.aisstream;if(!AISSTREAM_API_KEY){s.lastError='AISSTREAM_API_KEY is not configured';s.state='not_configured';return;}if(s.socket&&(s.socket.readyState===WebSocket.OPEN||s.socket.readyState===WebSocket.CONNECTING))return;if(s.cooldownUntil&&Date.parse(s.cooldownUntil)>Date.now())return;openSocket('aisstream','wss://stream.aisstream.io/v0/stream',(ws,state)=>{ws.send(JSON.stringify({APIKey:AISSTREAM_API_KEY,BoundingBoxes:BOXES,FilterMessageTypes:['PositionReport','ShipStaticData']}));state.lastSubscriptionAt=new Date().toISOString();},(d,state)=>{if(d.MessageType==='SubscriptionConfirmation'){state.state='subscribed';state.subscriptionConfirmed=true;state.compressionEnabled=d?.Message?.CompressionEnabled??null;state.reconnectAttempt=0;state.successfulSubscriptions++;return;}consume('aisstream',d);});}
...
app.listen(PORT,'0.0.0.0',()=>{console.log(`MaritimeScope Hybrid AIS bridge v10.3 listening on ${PORT}`);console.log(`Open Waters: ${!!OPENWATERS_API_KEY?'token':'anonymous'} | AISStream: ${!!AISSTREAM_API_KEY?'configured':'NOT configured'}`);console.log(`Boxes: ${JSON.stringify(BOXES)}`);setTimeout(()=>{connectOpenWaters();},2000); setTimeout(()=>{connectAISStream();},AISSTREAM_START_DELAY_MS);});
