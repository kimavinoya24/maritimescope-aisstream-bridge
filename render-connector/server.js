...
app.get('/',(req,res)=>res.json({ok:true,version:'9.0.0',name:'MaritimeScope AIS Bridge v9',service:AIS_PROVIDER==='openwaters'?'Open Waters aiscast':'AISStream',mode:PUSH_INGEST ? 'single-connection-with-optional-push-ingest' : 'single-connection-pull-mode',testMode:!ALLOW_GLOBAL_BOXES,boxes:BOXES,endpoints:['/health','/diagnostics','/vessels','/vessel','/search']}));
app.get('/health',(req,res)=>res.json({ok:true,version:'9.0.0',ingest:{enabled:PUSH_INGEST,inProgress:ingestInProgress,batchSize:INGEST_BATCH_SIZE,intervalMs:INGEST_INTERVAL_MS,retries:INGEST_RETRIES,pending:pending.size,successes:ingestSuccesses,failures:ingestFailures,rowsStored:ingestRowsStored,lastIngestAt,lastIngestStatus,lastIngestError,lastIngestDetail},aisConnected:!!socket&&socket.readyState===WebSocket.OPEN,subscriptionConfirmed,compressionEnabled,lastMessageAt,connectedAt,disconnectedAt,totalMessages,cacheSize:cache.size,reconnectAttempt,nextReconnectAt,lastClose,lastError,boxes:BOXES,testMode:!ALLOW_GLOBAL_BOXES}));
app.get('/diagnostics',(req,res)=>res.json({ok:true,version:'9.0.0',time:new Date().toISOString(),node:process.version,platform:process.platform,instance:{renderService:process.env.RENDER_SERVICE_NAME||null,renderInstance:process.env.RENDER_INSTANCE_ID||null},config:{provider:AIS_PROVIDER,apiKeyConfigured:!!AIS_KEY,apiKeyFingerprint:AIS_KEY?crypto.createHash('sha256').update(AIS_KEY).digest('hex').slice(0,12):null,bridgeTokenConfigured:!!BRIDGE_TOKEN,appUrlConfigured:!!APP_URL,ingestKeyConfigured:!!INGEST_KEY,pushIngestEnabled:PUSH_INGEST,allowGlobalBoxes:ALLOW_GLOBAL_BOXES,boxes:BOXES,maxCache:MAX_CACHE,ingestDebug:INGEST_DEBUG},connection:{mode:AIS_PROVIDER==='openwaters' ? (openWatersAnonymousFallback || !AIS_KEY ? 'anonymous' : 'token') : 'token',anonymousFallback:openWatersAnonymousFallback,anonymousFallbackAt:openWatersAuthFallbackAt,readyState:socket?socket.readyState:null,aisConnected:!!socket&&socket.readyState===WebSocket.OPEN,totalConnectAttempts,lastConnectStartedAt,connectedAt,disconnectedAt,subscriptionConfirmed,successfulSubscriptions,compressionEnabled,lastSubscriptionAt,reconnectAttempt,nextReconnectAt,lastClose,lastError,lastHandshake},data:{totalMessages,cacheSize:cache.size,lastMessageAt,pendingIngest:pending.size,ingest:{enabled:PUSH_INGEST,inProgress:ingestInProgress,batchSize:INGEST_BATCH_SIZE,intervalMs:INGEST_INTERVAL_MS,retries:INGEST_RETRIES,successes:ingestSuccesses,failures:ingestFailures,rowsStored:ingestRowsStored,lastIngestAt,lastIngestStatus,lastIngestError,lastIngestDetail}}}));
